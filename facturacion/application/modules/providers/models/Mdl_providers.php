...
class Mdl_providers extends Response_Model
{
    ...
    {
        $this->filter_select('IFnull((SELECT SUM(invoice_provider_total) FROM ip_invoice_provider_amounts WHERE invoice_provider_id IN (SELECT invoice_provider_id FROM ip_invoices_provider WHERE ip_invoices_provider.provider_id = ip_providers.provider_id)), 0) AS provider_invoice_total', false);
        return $this;
    }
