...
{
    public $table = 'ip_uploads_provider';
    public $primary_key = 'ip_uploads_provider.upload_id';
    public $date_modified_field = 'uploaded_date';

    public function default_order_by()
    {
        $this->db->order_by('ip_uploads_provider.upload_id ASC');
    }
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $row) {
                array_push($names, array(
                    'path' => UPLOADS_PFILES_FOLDER . $row->file_name_new,
    ...
    public function get_invoice_uploads($id)
    {
        ...
        $invoice = $this->mdl_invoices_provider->get_by_id($id);
        $query = $this->db->query("SELECT file_name_new,file_name_original FROM ip_uploads_provider WHERE url_key = '" . $invoice->invoice_provider_url_key . "'");
    ...
    public function delete_file($url_key, $filename)
    {
        ...
        $this->db->delete('ip_uploads_provider');
    }
    ...
    {
        $this->filter_where('ip_uploads_provider.provider_id', $provider_id);
        return $this;
    }
