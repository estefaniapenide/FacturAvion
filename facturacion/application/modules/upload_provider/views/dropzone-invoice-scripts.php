<script>
    ...
    var myDropzone = new Dropzone(document.body, { // Make the whole body a dropzone
        url: '<?php echo site_url('upload_provider/upload_file/' . $invoice->provider_id . '/' . $invoice->invoice_provider_url_key) ?>',
        params: {
        ...
        init: function () {
            thisDropzone = this;
            $.getJSON('<?php echo site_url('upload_provider/upload_file/' . $invoice->provider_id . '/' . $invoice->invoice_provider_url_key) ?>',
                function (data) {
                    $.each(data, function (index, val) {
                        var mockFile = {fullname: val.fullname, size: val.size, name: val.name};

                        thisDropzone.options.addedfile.call(thisDropzone, mockFile);
                        createDownloadButton(mockFile, '<?php echo site_url('upload_provider/get_file'); ?>/' + val.fullname);

                        if (val.fullname.match(/\.(jpg|jpeg|png|gif)$/)) {
                            thisDropzone.options.thumbnail.call(thisDropzone, mockFile,
                                '<?php echo site_url('upload_provider/get_file'); ?>/' + val.fullname);
                        }
                        else {
    ...
    myDropzone.on('addedfile', function (file) {
        ...
        myDropzone.emit('thumbnail', file,
            '<?php echo base_url('assets/core/img/file-icons/'); ?>' + fileIcon + '.svg');
        createDownloadButton(file, '<?php echo site_url('upload_provider/get_file/' . $invoice->invoice_provider_url_key . '_') ?>' +
    ...
    myDropzone.on('removedfile', function (file) {
        $.post({
            url: '<?php echo site_url('upload_provider/delete_file/' . $invoice->invoice_provider_url_key) ?>',
            data: {
