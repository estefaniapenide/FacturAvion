 ...
 */
class Mdl_Payment_Custom_Provider extends Validator
{
    public static $positions = array(
        'custom_fields'
    );
    public $table = 'ip_payment_custom_provider';
    public $primary_key = 'ip_payment_custom_provider.payment_custom_id';

    public function default_select()
    {
        $this->db->select('SQL_CALC_FOUND_ROWS ip_payment_custom_provider.*, ip_custom_fields.*', false);
    }

    public function default_join()
    {
        $this->db->join('ip_custom_fields', 'ip_payment_custom_provider.payment_custom_fieldid = ip_custom_fields.custom_field_id');
    }
    ...
    {
        $this->db->where('ip_payment_custom_provider.payment_id', $payment_id);
        return $this;
    }

    public function get_by_payid($payment_id)
    {
        $result = $this->where('ip_payment_custom_provider.payment_id', $payment_id)->get()->result();
        return $result;
    }
